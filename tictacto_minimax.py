...
def move(x, player):
    if valid_move(x):
        game_board[x] = player  # 위치 x가 비어 있으면 해당 위치에 player가 둠
        return True
    return False
...
def minimax(board, depth, maxPlayer):
    ...
    if depth == 0 or len(empty_cells(board)) == 0 or game_over(board):
        # 게임이 끝났거나, 더 이상 둘 곳이 없으면 -1과 평가값을 리턴하고 게임 종료
        return -1, evaluate(board)

    if maxPlayer:
        value = -10000  # 음의 무한대
        # 자식 노드를 하나씩 평가해서 최선을 수(MAX)를 찾는다.
        for p in empty_cells(board):
            board[p] = 'X'  # 보드의 p 위치에 'X'을 놓아보고,

            # 경기자를 교체해서 minimax()를 순환호출한다.
            x, score = minimax(board, depth - 1, False)
            board[p] = ' '  # 보드는 원 상태로 돌린다.
            if score > value:  # p에 뒀을 때 max값이 되면 그 위치를 기억해야 해
                value = score  # 최대값을 취한다.
                pos = p  # 최대값의 위치를 기억한다.
    else:
        value = +10000  # 양의 무한대
        # 자식 노드를 하나씩 평가하여서 최선의 수(min)를 찾는다.
        for p in empty_cells(board):
